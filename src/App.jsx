        ...
        {!loading && tab==="admin" && adminMode && (
          <div>
            ...
            <div style={{background:"#0A1420",border:"1px solid #1A3050",borderRadius:12,padding:12,marginBottom:16,fontSize:11,color:"#4A6080",lineHeight:1.8}}>
              ...
              <div>1️⃣ Match starts → tap <b style={{color:"#EF4444"}}>🔒 Lock Bets</b></div>
              <div>2️⃣ Set toss winner after coin toss</div>
              <div>3️⃣ Match ends → tap winning team</div>
              <div>4️⃣ Points update on all phones instantly ✅</div>
            </div>
            {matches.map((match,idx)=>{
              const status=getEffectiveStatus(match);
              const manual=manualResults[fbKey(match.id)]||{};
              ...
              return(
                <div key={match.id} style={{...S.card("#1A3050"),marginBottom:10,opacity:status==="completed"?0.85:1}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                    ...
                    <div style={{fontSize:10,fontWeight:700,padding:"3px 8px",borderRadius:20,background:statusConfig.bg,color:statusConfig.color}}>{statusConfig.label}</div>
                  </div>
                  <div style={{fontSize:10,color:"#2A4060",marginBottom:10}}>📅 {match.date} · {match.time} · {match.venue.split(",")[0]}</div>
                  ...
                  {status!=="completed"&&(
                    <div>
                      ...
                      <div style={{display:"flex",gap:8,marginBottom:10}}>
                        {[match.home,match.away].map(team=>(
                          <button key={team} onClick={()=>setManualResult(match.id,manual.winner||null,team,manual.status||status)}
                            style={S.pill(manual.tossWinner===team,"#FFD700")}>
                      ...
                      <div style={{display:"flex",gap:8,marginBottom:10}}>
                        {[match.home,match.away].map(team=>(
                          <button key={team} onClick={()=>setManualResult(match.id,team,manual.tossWinner||null,"completed")}
                            style={S.pill(manual.winner===team,"#22C55E")}>
